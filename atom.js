...
* @returns {String} The boilerplate feed as a string
...
var boilerplate = function (details) {
    ...
    element = function (elem, n, content, atts) {
        return indent(n) + '<' + elem + (atts || '') + (content ? '>' + content + '</' + elem + '>' : '/>') + '\n';
    },
    parent = function (elem, n, content, atts) {
        var ind = indent(n);
        ...
        return function (prev, elem) {
            ...
            return prev + (container[elem] ?
                (!stringContainer && container[elem].type === 'xhtml' ?
                    element(elem, level, '<div xmlns="http://www.w3.org/1999/xhtml">' + (stringContainer || container[elem].content) + '</div>') :
                    element(elem, level, escapeContent(stringContainer || container[elem]))
                ) : '');
        };
    },
    undefinedContentElement = function (parent, level, atts, elemName, elemsName) {
        ...
        return elems.reduce(function (prev, elem) { // We don't escape "elem" as it could be XML
            ...
            return prev + element(elemName, level, elem.content, escapeAttributes(atts, elem));
        }, '');
    },
    ...
    getPersons = function (container, level) {
        return [
            ...
            return prev + persons.reduce(function (prev, person) {
                ...
                return prev +
                    parent(personType, level,
                        [
                            'name', // Required for author if present // Validation above
                            'uri',
                            'email'
                        ].reduce(function (prev, elem) {
                            return prev + escapeElement(person, elem, nextLevel);
                        }, '') +
                        (exts ? (indent(nextLevel) + exts + '\n') : '')
                    );
            }, '');
        }, '');
    },
    ...
    */
    feedOrSource = function (parentObj, level, isFeed) {
        ...
        if (isFeed && // <feed>
            ...
            throw 'The authors requirement was not met.';
        }
        ...
        return [
            ...
            return prev + escapeElement(parentObj, elem, level);
        }, '') +

        element('updated', level, formatDate(parentObj.updated)) + // REQUIRED (format: 2003-12-13T18:30:02Z)
        ...
        (generator ? // We'll add our generator by default, but allow omission
            element('generator', level, escapeContent(generator),
                escapeAttributes([
        ...
        (parentObj.extensionElements ? indent(level) + parentObj.extensionElements + '\n' : '') +
        ...
        return parentObj.entries.reduce(function (prev, entry) {
            ...
            return prev +
                parent('entry', level,
                    ...
                    escapeElement(entry, 'id', nextLevel) + // REQUIRED (IRI)

                    [
                        'updated', // REQUIRED (format: 2003-12-13T18:30:02Z)
                        'published' // (created or availability date)
                    ].reduce(function (prev, elem) {
                        return prev + element(elem, nextLevel, formatDate(entry[elem]));
                    }, '') +
                    ...
                    (entry.extensionElements ? indent(nextLevel) + entry.extensionElements + '\n' : '') +

                    (entry.source ? parent('source', nextLevel, feedOrSource(entry.source, nextLevel)) : '') +

                    // "src" is apparently not supported in Firefox
                    // Todo: Clean this up (and any other still verbose constructs)
                    (src ?
                        element('content', nextLevel, '', escapeAttribute('type', entry.content && typeof entry.content === 'object' && entry.content.type) + escapeAttribute('src', src)) : // content (@src (if present should be with @type=MIME and no content), otherwise type=text|html|xhtml (with h:div for xhtml))
                        (innerContent ?
                            ((entry.content && entry.content.type === 'xhtml') ?
                                parent('content', nextLevel,
                                    element('div', nextLevel + 1, innerContent, attribute('xmlns', 'http://www.w3.org/1999/xhtml')), // Do not escape
                                    escapeAttribute('type', entry.content && entry.content.type)) :
                                    element('content', nextLevel,
                                        ...
                                        (!entry.content.type || entry.content.type === 'html' || entry.content.type === 'text' || entry.content.type.match(/^text\//)))) ?
                                            escapeXMLContent(innerContent) : // text or html
                                    ...
                                    escapeAttribute('type', entry.content && typeof entry.content === 'object' && entry.content.type))) : '')
                    ),
                    atts
                );
        }, '');
    ...
    rootAttributes = escapeAttribute('xmlns', 'http://www.w3.org/2005/Atom') + escapeAttribute('xml:base', details.base) +
        ((details.lang || (details.lang !== false && defaults.lang)) ? escapeAttribute('xml:lang', details.lang || defaults.lang) : ''); // We'll add English by default, but allow omission
    ...
    try {
        ['id', 'title', 'updated'].forEach(function (requiredProperty) {
            if (!details[requiredProperty]) { // Todo: change to return error code number?
                throw 'The required property ' + requiredProperty + ' was not supplied, or, if a single entry feed, ';
            }
        });
    }
    catch (e) {
        ...
        return xmlDeclaration + getEntries(details, 0, rootAttributes);
    }
